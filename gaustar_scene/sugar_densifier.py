...
class SuGaRDensifier():
    ...
    # def densify_detection_only(self, grads, max_grad):
    ...
    def densify_detection(self, max_grad: float=None, return_grad=False):
        max_grad = self.max_grad if max_grad is None else max_grad
        ...
        # Extract points that satisfy the gradient condition
        selected_pts_mask = torch.where(grads >= max_grad, True, False)
        if return_grad:
            return selected_pts_mask, grads
        return selected_pts_mask
