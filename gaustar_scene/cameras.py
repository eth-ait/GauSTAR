...
from gaustar_utils.graphics_utils import focal2fov, fov2focal, getWorld2View2, getProjectionMatrix
from gaustar_utils.general_utils import PILtoTorch
...
def load_gs_cameras(source_path, gs_output_path, image_resolution=1, 
                    load_gt_images=True, max_img_size=4092, from_humanrf=False):  # 1920  2046  4092
    ...
    for cam_idx in tqdm(range(len(camera_transforms))):
        ...
        if load_gt_images:
            ...
            # mask_path = img_path.parent.parent / "masks" / f"{name}_alpha.png"
            mask_path = source_path + f"masks{label}/{name}_alpha.png"
            loaded_mask = PILtoTorch(Image.open(mask_path), resolution)

            # if os.path.exists(source_path + f"depth{label}/{name}_depth.tiff"):
            #     depth_img = tf.imread(source_path + f"depth{label}/{name}_depth.tiff")
            if os.path.exists(source_path + f"depth{label}/{name}_depth.npz"):
                depth_img = np.load(source_path + f"depth{label}/{name}_depth.npz")['depth']
            else:
                raise RuntimeError("Depth image not found")
            loaded_depth = PILtoTorch(Image.fromarray(depth_img), resolution, use_scalar=False, default_sample=False)
        ...
        gs_camera = GSCamera(
            colmap_id=id, image=gt_image, gt_alpha_mask=loaded_mask, gt_depth=loaded_depth,
    ...
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        if image is None:
            ...
            if gt_alpha_mask is not None:
                ...
                # ZCW add mask
                gt_alpha_mask_dev = gt_alpha_mask[0].to(self.data_device)
                self.original_image[0] *= gt_alpha_mask_dev
                self.original_image[1] = self.original_image[1] * gt_alpha_mask_dev + 1 * (1 - gt_alpha_mask_dev)
                # self.original_image[1] *= gt_alpha_mask_dev
                self.original_image[2] *= gt_alpha_mask_dev
                # img = self.original_image.detach().cpu().numpy().transpose(1, 2, 0)[..., ::-1]

                # self.original_mask = gt_alpha_mask_dev[None]
            else:
...
def convert_camera_from_gs_to_pytorch3d(gs_cameras, device='cuda'):
    ...
    # ZCW change
    focal_pytorch3d = torch.cat((fx[..., None], fy[..., None]), dim=1).to(device) / scale
    p0_pytorch3d = p0_pytorch3d.expand(N, -1)
    K = _get_sfm_calibration_matrix(N, "cpu", focal_pytorch3d, p0_pytorch3d, orthographic=False)
